...
brief:  this script trains a DeepLabV3+ blood segmentation model on all
    ...
    Every configuration contains a separate train.csv and validation.csv.
...
import torch
from src.data_transforms import create_train_transform, create_eval_transform
...
# lab/k_fold/training/train_deeplabv3plus_all_config.py
...
if len(sys.argv) > 3:
    raise ValueError(
        "Usage: python -m k_fold.training.train_deeplabv3plus_all_config "
...
def create_data_loaders(configuration_directory):
    ...
    return train_hemo_DL, valid_hemo_DL
...
for configuration_directory in configuration_directories:
    ...
    train_hemo_DL, valid_hemo_DL = create_data_loaders(configuration_directory)

    print(f"train images: {len(train_hemo_DL.dataset)}")
    print(f"validation images: {len(valid_hemo_DL.dataset)}")
    ...
    for epoch in range(n_epochs):
        ...
        for i, (train_img, train_mask) in enumerate(train_hemo_DL):
        ...
        with torch.no_grad():
            for j, (val_img, val_mask) in enumerate(valid_hemo_DL):
