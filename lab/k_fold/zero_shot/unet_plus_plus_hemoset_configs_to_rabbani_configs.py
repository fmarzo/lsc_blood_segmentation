...
# lab/k_fold/zero_shot/unet_plus_plus_hemoset_configs_to_rabbani_configs.py
lab_directory = Path(__file__).resolve().parents[2]

splits_directory = lab_directory / "k_fold" / "rabbani" / "generated_splits"
...
if len(sys.argv) > 2:
    raise ValueError(
        "Usage: python -m k_fold.zero_shot.unet_plus_plus_hemoset_configs_to_rabbani_configs "
...
def get_checkpoint_path(configuration_id):
    return (
        checkpoint_directory
        / f"unet_plus_plus_{segmentation_mode}_best_"
...
print("source dataset: HemoSet")
print("target dataset: Rabbani")
