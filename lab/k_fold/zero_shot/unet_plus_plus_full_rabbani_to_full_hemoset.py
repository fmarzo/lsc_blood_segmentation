"""
file: unet_plus_plus_full_rabbani_to_full_hemoset.py

brief:  this script trains U-Net++ with a ResNet-18 encoder on the complete
        Rabbani dataset divided into train and validation, then
        performs zero-shot evaluation on the complete HemoSet
...
source_train_csv = lab_directory / "k_fold" / "zero_shot" / "split_dataset" / "generated" / "rabbani" / "train.csv"
source_validation_csv = lab_directory / "k_fold" / "zero_shot" / "split_dataset" / "generated" / "rabbani" / "validation.csv"
target_full_csv = lab_directory / "splits" / "full_labeled_dataset.csv"
SOURCE_TRAIN_TRANSFORM = create_bleed_train_transform
SOURCE_EVAL_TRANSFORM = create_bleed_eval_transform
TARGET_EVAL_TRANSFORM = create_eval_transform
SOURCE_DATASET_NAME = "rabbani"
TARGET_DATASET_NAME = "hemoset"
...
if len(sys.argv) > 1 and sys.argv[1] != "":
    ...
    n_epochs = 50
if len(sys.argv) > 2:
    raise ValueError("Usage: python -m k_fold.zero_shot.unet_plus_plus_full_rabbani_to_full_hemoset [epochs]")
if n_epochs <= 0:
...
checkpoint_path = (
    checkpoint_directory
    / f"unet_plus_plus_zero_shot_full_rab_to_full_hemo_{segmentation_mode}_best_{encoder_name}.pth"
)
