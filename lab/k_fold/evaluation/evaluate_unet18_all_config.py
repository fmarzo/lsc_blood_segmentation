"""
file: evaluate_unet18_all_config.py

brief:  this script evaluates the best U-Net ResNet-18 checkpoint associated
        with every selected grouped configuration.
    ...
    was previously selected by train_unet18_all_config.py using the lowest
    validation loss.
    ...
    variance and sample standard deviation of the test metrics across the
    configurations.
...
if len(sys.argv) > 2:
    raise ValueError(
        "Usage: python -m k_fold.evaluation.evaluate_unet18_all_config "
...
brief:    this routine creates the model architecture used by the checkpoint
"""
def create_model():
    unet = smp.Unet(
...
for configuration_directory in configuration_directories:
    ...
    checkpoint_path = (
        checkpoint_directory
        / f"unet_{segmentation_mode}_best_{encoder_name}_config_{configuration_id:03d}.pth"
    )

    test_hemo_DL = create_test_loader(configuration_directory)

    print(f"test images: {len(test_hemo_DL.dataset)}")

    unet = create_model()
    checkpoint = load_checkpoint(unet, checkpoint_path)
    ...
    with torch.no_grad():
        for i, (test_img, test_mask) in enumerate(test_hemo_DL):
            ...
            if i % 50 == 0:
                print(f"evaluated batch {i}/{len(test_hemo_DL)}")
    ...
    test_result = {
        ...
        "test_images": len(test_hemo_DL.dataset),
        **test_metrics,
    }
