"""
file: evaluate_unet_plus_plus_all_config.py

brief:  this script evaluates the best U-Net++ ResNet-18 checkpoint
        associated with every selected grouped configuration.
    ...
    was previously selected by train_unet_plus_plus_all_config.py using the lowest validation
    loss.

    The matching config_XXX/test.csv file is loaded and evaluated exactly once.

    The model uses the same nested U-Net++ decoder and ResNet-18 encoder
...
# lab/k_fold/evaluation/evaluate_unet_plus_plus_all_config.py
...
if len(sys.argv) > 2:
    raise ValueError(
        "Usage: python -m k_fold.evaluation.evaluate_unet_plus_plus_all_config "
...
for configuration_directory in configuration_directories:
    ...
    checkpoint_path = (
        checkpoint_directory
        / f"unet_plus_plus_{segmentation_mode}_best_{encoder_name}_config_{configuration_id:03d}.pth"
    )
