...
brief:  this script evaluates the best U-Net ResNet-18 checkpoint associated
        with every selected stratified Rabbani configuration.
...
# lab/k_fold/rabbani/evaluation/evaluate_unet18_all_config.py
...
if len(sys.argv) > 2:
    raise ValueError(
        "Usage: python -m k_fold.rabbani.evaluation.evaluate_unet18_all_config "
...
for configuration_directory in configuration_directories:
    ...
    checkpoint_path = (
        checkpoint_directory
        / f"unet_rab_{segmentation_mode}_best_{encoder_name}_config_{configuration_id:03d}.pth"
    )
