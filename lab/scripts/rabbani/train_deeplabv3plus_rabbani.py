...
def train_one_epoch(model, train_loader, optimizer, bce_loss, dice_loss, cross_entropy_loss):
    """
    ...
    optimizer_steps = 0
    for batch_index, (train_images, train_masks) in enumerate(train_loader):
        ...
        if not torch.isfinite(loss):
            raise RuntimeError("Non-finite training loss detected " f"at batch {batch_index}: " f"{loss.item()}")
        loss.backward()
        gradient_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=GRADIENT_CLIP_MAX_NORM)
        optimizer.step()
        ...
        optimizer_steps += 1
        if batch_index % 50 == 0:
            print(f"train batch " f"{batch_index}/{len(train_loader)} " f"loss {loss.item():.6f}")
    average_loss = running_loss / number_of_samples
    average_gradient_norm = gradient_norm_sum / max(optimizer_steps, 1)
    return {"loss": average_loss, "gradient_norm": average_gradient_norm}


def validate_model(model, validation_loader, bce_loss, dice_loss, cross_entropy_loss):
    """
    ...
    with torch.inference_mode():
        for batch_index, (validation_images, validation_masks) in enumerate(validation_loader):
            ...
            if not torch.isfinite(loss):
                raise RuntimeError("Non-finite validation loss detected " f"at batch {batch_index}: " f"{loss.item()}")
            ...
            number_of_samples += current_batch_size
            if batch_index % 50 == 0:
                print(f"validation batch " f"{batch_index}/{len(validation_loader)} " f"loss {loss.item():.6f}")
    validation_loss = running_loss / number_of_samples
...
if len(sys.argv) >= 2:
    maximum_epochs = int(sys.argv[1])
else:
    maximum_epochs = getattr(config_split, "DEFAULT_EPOCHS", 50)
if maximum_epochs <= 0:
...
checkpoint_path = os.path.join(
    ...
    (f"deeplabv3plus_{SEGMENTATION_MODE}_" "best_dice_bleed_seg_" f"{ENCODER_NAME}_rab.pth"),
)

# DATASET AND TRANSFORMS

train_transform = create_bleed_train_transform()
validation_transform = create_bleed_eval_transform()
train_dataset = CustomImageDataset(config_split.CSV_TRAIN_PATH_V1P0, train_transform)
validation_dataset = CustomImageDataset(config_split.CSV_VALID_PATH_V1P0, validation_transform)
...
optimizer = torch.optim.AdamW(
    [
        ...
        {"params": (model.segmentation_head.parameters()), "lr": DECODER_LEARNING_RATE},
    ],
...
print("======== RABBANI DEEPLABV3+ " "TRAINING CONFIGURATION ========")
print(f"Training CSV: " f"{config_split.CSV_TRAIN_PATH_V1P0}")
print(f"Validation CSV: " f"{config_split.CSV_VALID_PATH_V1P0}")
print(f"Training samples: " f"{len(train_dataset)}")
print(f"Validation samples: " f"{len(validation_dataset)}")
print(f"Training batches: " f"{len(train_loader)}")
print(f"Validation batches: " f"{len(validation_loader)}")
...
print(f"Maximum epochs: " f"{maximum_epochs}")
print(f"Batch size: " f"{BATCH_SIZE}")
...
for epoch_index in range(maximum_epochs):
    ...
    print(f"\n------------ EPOCH: " f"{current_epoch}/{maximum_epochs} " f"------------")
    ...
    current_selection_score = validation_results["selection_score"]
    if not np.isfinite(current_selection_score):
        raise FloatingPointError("Non-finite validation selection score detected.")
    
    # CHECKPOINT SELECTION
    
    meaningful_improvement = current_selection_score > best_selection_score + MIN_CHECKPOINT_IMPROVEMENT
    if meaningful_improvement:
        best_selection_score = current_selection_score
        best_global_dice = validation_results["global_dice"]
        best_mean_image_dice = validation_results["mean_image_dice"]
        ...
        print(f"Epochs without improvement: " f"{epochs_without_improvement}/" f"{EARLY_STOPPING_PATIENCE}")
    
    # SCHEDULER
    
    scheduler.step(current_selection_score)
    ...
    print("\n----- Training -----")
    print(f"Loss: " f"{training_results['loss']:.6f}")
    print(f"Average gradient norm: " f"{training_results['gradient_norm']:.6f}")
    print("\n----- Validation -----")
    print(f"Loss: " f"{validation_results['loss']:.6f}")
    print(f"Global IoU: " f"{validation_results['global_iou']:.6f}")
    print(f"Global Dice: " f"{validation_results['global_dice']:.6f}")
    print(f"Global precision: " f"{validation_results['global_precision']:.6f}")
    print(f"Global recall: " f"{validation_results['global_recall']:.6f}")
    print(
        f"Mean-image IoU: "
    ...
    print(f"Selection score: " f"{current_selection_score:.6f}")
...
print("\n======== RABBANI DEEPLABV3+ " "TRAINING COMPLETED ========")
