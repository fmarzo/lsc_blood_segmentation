...
brief:
    ...
    The model architecture is selected in src/config_split.py through:RABBANI_EVALUATION_MODEL = "unet_plus_plus" 
    or: RABBANI_EVALUATION_MODEL = "deeplabv3plus"

    The segmentation mode is selected through: SEGMENTATION_MODE = "binary"
    or: SEGMENTATION_MODE = "multiclass"
...
if not torch.cuda.is_available():
    raise RuntimeError("CUDA is not available. " "This script is configured for GPU evaluation.")
...
def create_model_and_checkpoint():
    ...
    if MODEL_NAME == "unet_plus_plus":
        model = smp.UnetPlusPlus(
            encoder_name=ENCODER_NAME, encoder_weights=None, in_channels=3, classes=NUM_OUTPUT_CHANNELS, activation=None
        )
        checkpoint_filename = f"unet_plus_plus_{SEGMENTATION_MODE}_" "best_dice_bleed_seg_" f"{ENCODER_NAME}_rab.pth"
        ...
        checkpoint_filename = f"deeplabv3plus_{SEGMENTATION_MODE}_" "best_dice_bleed_seg_" f"{ENCODER_NAME}_rab.pth"
        model_display_name = "DeepLabV3+"
    checkpoint_path = os.path.join(config_split.MODEL_PRETRAINED_DIR, checkpoint_filename)
    return (model.to(DEVICE), checkpoint_path, model_display_name)
...
def load_checkpoint(model, checkpoint_path):
    ...
    if not os.path.isfile(checkpoint_path):
        raise FileNotFoundError("Rabbani checkpoint not found: " f"{checkpoint_path}")
...
model, checkpoint_path, model_display_name = create_model_and_checkpoint()
...
test_dataset = CustomImageDataset(config_split.CSV_TEST_PATH, eval_transform)


if len(test_dataset) == 0:
    raise ValueError("The HemoSet test dataset is empty: " f"{config_split.CSV_TEST_PATH}")


if config_split.VIDEOID_STRING not in test_dataset.csv_dirs.columns:
    raise KeyError("Video ID column not found in the HemoSet test CSV: " f"{config_split.VIDEOID_STRING}")
...
print("======== RABBANI MODEL TO HEMOSET " "ZERO-SHOT EVALUATION ========")

print(f"Model: {model_display_name}")

print(f"Model configuration value: " f"{MODEL_NAME}")

print(f"Encoder: {ENCODER_NAME}")


if MODEL_NAME == "deeplabv3plus":
    print(f"Encoder output stride: " f"{ENCODER_OUTPUT_STRIDE}")
    print(f"Decoder channels: " f"{DECODER_CHANNELS}")
    print(f"Decoder atrous rates: " f"{DECODER_ATROUS_RATES}")
...
print("Training dataset: " "Rabbani Bleeding Segmentation")

print("Evaluation dataset: " "HemoSet test split")

print(f"Checkpoint: " f"{checkpoint_path}")

print(f"Test CSV: " f"{config_split.CSV_TEST_PATH}")

print(f"Test images: " f"{len(test_dataset)}")

print(f"Test batches: " f"{len(test_loader)}")
...
with torch.inference_mode():
    for batch_index, (test_images, test_masks) in enumerate(test_loader):
        test_images = test_images.to(DEVICE, non_blocking=True)
        test_masks = prepare_mask(test_masks)
        logits = model(test_images)
        if batch_index == 0:
            validate_output_shape(logits, test_masks)
            print(f"Input batch shape: " f"{tuple(test_images.shape)}")
            print(f"Output batch shape: " f"{tuple(logits.shape)}")
            print(f"Mask batch shape: " f"{tuple(test_masks.shape)}")
        predictions = get_predictions(logits)
        batch_tp, batch_fp, batch_fn, batch_tn = get_segmentation_stats(predictions, test_masks)
        tp_batches.append(batch_tp.cpu())
        fp_batches.append(batch_fp.cpu())
        fn_batches.append(batch_fn.cpu())
        tn_batches.append(batch_tn.cpu())
        if batch_index % 50 == 0:
            print(f"Evaluated batch " f"{batch_index}/" f"{len(test_loader)}")
...
iou_classes, dice_classes, precision_classes, recall_classes = compute_metrics(tp, fp, fn, tn)

iou_per_image = iou_classes[:, BLOOD_CLASS_INDEX]
dice_per_image = dice_classes[:, BLOOD_CLASS_INDEX]
precision_per_image = precision_classes[:, BLOOD_CLASS_INDEX]
recall_per_image = recall_classes[:, BLOOD_CLASS_INDEX]
...
std_precision = precision_per_image.std(correction=0).item()

mean_recall = recall_per_image.mean().item()
std_recall = recall_per_image.std(correction=0).item()
...
blood_gt_pixels = tp[:, BLOOD_CLASS_INDEX] + fn[:, BLOOD_CLASS_INDEX]

blood_pred_pixels = tp[:, BLOOD_CLASS_INDEX] + fp[:, BLOOD_CLASS_INDEX]
...
correct_empty_predictions = (empty_images_mask & (blood_pred_pixels == 0)).sum().item()

empty_images_with_false_positives = (empty_images_mask & (blood_pred_pixels > 0)).sum().item()


# HEMOSET VIDEO IDENTIFIERS


test_video_ids = test_dataset.csv_dirs[config_split.VIDEOID_STRING].astype(str)

video_ids = sorted(test_video_ids.unique(), key=extract_video_number)
...
for video_id in video_ids:
    video_mask = torch.tensor((test_video_ids == str(video_id)).to_numpy(), dtype=torch.bool)
    video_image_count = video_mask.sum().item()
    if video_image_count == 0:
        continue
    video_iou_per_image = iou_per_image[video_mask]
    video_dice_per_image = dice_per_image[video_mask]
    video_precision_per_image = precision_per_image[video_mask]
    video_recall_per_image = recall_per_image[video_mask]
    ...
    video_iou_classes, video_dice_classes, video_precision_classes, video_recall_classes = compute_metrics(
        video_tp, video_fp, video_fn, video_tn
    )
    video_global_iou = video_iou_classes[BLOOD_CLASS_INDEX].item()
    video_global_dice = video_dice_classes[BLOOD_CLASS_INDEX].item()
    video_global_precision = video_precision_classes[BLOOD_CLASS_INDEX].item()
    video_global_recall = video_recall_classes[BLOOD_CLASS_INDEX].item()
    video_mean_iou = video_iou_per_image.mean().item()
    video_std_iou = video_iou_per_image.std(correction=0).item()
    video_mean_dice = video_dice_per_image.mean().item()
    video_std_dice = video_dice_per_image.std(correction=0).item()
    print(f"\nVideo: " f"{video_id}")
    print(f"Images: " f"{video_image_count}")
    print("Per-image IoU:       " f"{video_mean_iou:.4f} " f"+/- {video_std_iou:.4f}")
    print("Per-image Dice:      " f"{video_mean_dice:.4f} " f"+/- {video_std_dice:.4f}")
    print(f"Global IoU:       " f"{video_global_iou:.4f}")
    print(f"Global Dice:      " f"{video_global_dice:.4f}")
    print(f"Global Precision: " f"{video_global_precision:.4f}")
    print(f"Global Recall:    " f"{video_global_recall:.4f}")
...
print("\n======== RABBANI MODEL TO HEMOSET " "ZERO-SHOT RESULTS ========")
...
print(f"IoU:       " f"{global_iou:.4f}")
print(f"Dice:      " f"{global_dice:.4f}")


print("\n----- Per-image blood metrics -----")

print(f"IoU:       " f"{mean_iou:.4f} " f"+/- {std_iou:.4f}")
print(f"Dice:      " f"{mean_dice:.4f} " f"+/- {std_dice:.4f}")
